...
bosox = pd.read_csv(os.path.join(raw_data_dir, 'BOS_1932_schedule.csv'), usecols=['Gm#', 'W/L', 'R', 'RA'])

# rename columns in sox and mets
for team in [sox, mets, bosox]:
...
for team in [rockies, sox, mets, bosox]:
    ...
    team['RD'] = team['R'].cumsum() - team['RA'].cumsum() # track the cumulative Run Differential

# join everyone together
losers = pd.concat([rockies, sox, mets, bosox], ignore_index=True)
